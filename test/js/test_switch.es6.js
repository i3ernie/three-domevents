...
function logOnClick( ev ){
    console.log( "target.name " + ev.target.name + " = this.name " + this.name );
...
function remove( ev ){
    ...
    if ( DEH.getEventGroupName() === "inter" ){
        DEH.resetEventGroup();
        console.log("reset event group");
    } else {
        DEH.switchEventGroup("inter");
        console.log("set event group: inter");
    }
